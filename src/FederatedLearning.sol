...
//        require(evalflag[_round] == false,"evaluation completed already.");
    ...
    struct Aggregator {
        address account;
        bool isActive;
        uint256 correctAggregations;
        uint256 rejectedAggregations;
    }
    
    // 聚合提案結構
    struct AggregationProposal {
        bytes32 modelCid;
        address proposer;
    ...
    address[] public aggregatorList;
    uint256 public minRequiredApprovals;
    
    // 每輪的聚合提案
    mapping(uint256 => AggregationProposal) public roundAggregationProposals;
    
    // 事件
    event AggregationProposed(uint256 indexed round, bytes32 modelCid, address proposer);
    event AggregationVoted(uint256 indexed round, address voter, bool approved);
    event AggregationFinalized(uint256 indexed round, bytes32 modelCid, bool accepted);
    
    // 修飾器：只有已註冊的聚合者可以調用
    modifier onlyAggregator() {
        require(aggregators[tx.origin].isActive, "Not an active aggregator");
        _;
    }
    
    // 新增：註冊聚合者的功能
    function registerAggregator(address _aggregator) external evaluatorOnly {
        require(!aggregators[_aggregator].isActive, "Already an aggregator");
        
        aggregators[_aggregator] = Aggregator({
            account: _aggregator,
            isActive: true,
            correctAggregations: 0,
            rejectedAggregations: 0
        });
        
        aggregatorList.push(_aggregator);
        
        // 更新所需的最小批准數量（例如：超過一半的聚合者）
        minRequiredApprovals = (aggregatorList.length / 2) + 1;
    }
    
    // 新增：提交聚合提案的功能
    function proposeAggregation(uint256 _round, bytes32 _modelCid) external onlyAggregator {
        require(_round == currentRound(), "Can only propose for current round");
        require(updatesInRound[_round].length >= maxNumUpdates, "Not enough updates to aggregate");
        require(!roundAggregationProposals[_round].isFinalized, "Round already has finalized aggregation");
        
        AggregationProposal storage proposal = roundAggregationProposals[_round];
        
        // 如果這是第一個提案，初始化它
        if (proposal.timestamp == 0) {
            proposal.modelCid = _modelCid;
            proposal.proposer = tx.origin;
            proposal.timestamp = block.timestamp;
            
            // 提案者自動投贊成票
            proposal.approvalCount = 1;
            proposal.hasVoted[tx.origin] = true;
            proposal.approved[tx.origin] = true;
        } else {
            // 如果已經有提案，這是一個矛盾的提案
            require(proposal.modelCid != _modelCid, "Same aggregation already proposed");
            require(!proposal.hasVoted[tx.origin], "Already voted on this round");
            
            // 記錄反對票
            proposal.rejectionCount++;
            proposal.hasVoted[tx.origin] = true;
            proposal.approved[tx.origin] = false;
        }
        
        emit AggregationProposed(_round, _modelCid, tx.origin);
    }
    
    // 新增：對聚合提案投票的功能
    function voteOnAggregation(uint256 _round, bool _approve) external onlyAggregator {
        AggregationProposal storage proposal = roundAggregationProposals[_round];
        require(proposal.timestamp > 0, "No proposal exists for this round");
        require(!proposal.isFinalized, "Aggregation already finalized");
        require(!proposal.hasVoted[tx.origin], "Already voted");
        
        proposal.hasVoted[tx.origin] = true;
        proposal.approved[tx.origin] = _approve;
        
        if (_approve) {
            proposal.approvalCount++;
            
            // 如果達到閾值，自動完成
            if (proposal.approvalCount >= minRequiredApprovals) {
                _finalizeAggregation(_round, true);
            }
        } else {
            proposal.rejectionCount++;
            
            // 如果拒絕票達到無法通過的數量，自動拒絕
            if (proposal.rejectionCount > aggregatorList.length - minRequiredApprovals) {
                _finalizeAggregation(_round, false);
            }
        }
        
        emit AggregationVoted(_round, tx.origin, _approve);
    }
    
    // 內部函數：完成聚合流程
    function _finalizeAggregation(uint256 _round, bool _accepted) internal {
        AggregationProposal storage proposal = roundAggregationProposals[_round];
        proposal.isFinalized = true;
        
        if (_accepted) {
            // 更新全局模型
            globalmodel[_round] = proposal.modelCid;
            
            // 更新提案者的統計數據
            aggregators[proposal.proposer].correctAggregations++;
            ...
            // 更新提案者的統計數據
            aggregators[proposal.proposer].rejectedAggregations++;
        }
        
        emit AggregationFinalized(_round, proposal.modelCid, _accepted);
    }
    
    // 新增：獲取聚合提案狀態的功能
    function getAggregationStatus(uint256 _round) external view returns (
        bytes32 modelCid,
        address proposer,
        uint256 approvalCount,
        uint256 rejectionCount,
    ...
    // 新增：檢查地址是否已對某輪投票
    function hasVotedOnAggregation(uint256 _round, address _voter) external view returns (bool, bool) {
        AggregationProposal storage proposal = roundAggregationProposals[_round];
        return (proposal.hasVoted[_voter], proposal.approved[_voter]);
    }
    
    // 修改現有的 saveGlobalmodel 函數
    function saveGlobalmodel(bytes32 _cid, uint256 _round) external {
        require(
            tx.origin == evaluator || 
            (aggregators[tx.origin].isActive && roundAggregationProposals[_round].isFinalized && 
             roundAggregationProposals[_round].modelCid == _cid),
            "Not authorized to save global model"
        );
        globalmodel[_round] = _cid;
    }
}